...
class OptimCVXPY(BaseAgent):
    ...
    def __init__(
        ...
        chosen = list(range(2, 7)) + list(range(7, 193)) + list(range(193, 466)) + list(range(466, 1954))
        #       label      timestamp         generator-PQV            load-PQV               line-PQUI
        chosen += list(range(1954, 2140)) + list(range(2140, 2326)) + list(range(2326, 2512)) + list(range(2512, 3044))
        #               line-rho               line switch         line-overload steps          bus switch
        chosen += list(range(3044, 3230)) + list(range(3230, 3348)) + list(range(3348, 3534)) + list(range(3534, 3720))
        #          line-cool down steps   substation-cool down steps     next maintenance         maintenance duration
        self.max_array = np.load(os.path.join(this_directory_path, "max_array.npy"))
        self.min_array = np.load(os.path.join(this_directory_path, "min_array.npy"))
    ...
    def run_dc(self, obs: BaseObservation):
        ...
        has_converged = self._solve_problem(prob, solver_type=cp.SCS)
    ...
    def change_substation_topology(self, observation, base_action, min_rho=None):
        ...
        features = np.divide((features[:] - self.min_array[1:]), (self.max_array[1:] - self.min_array[1:]), features[:],
                             where=(self.max_array[1:] - self.min_array[1:]) != 0)
        a_pred = self.ppo.predict_step(features)
        a_pred = a_pred._numpy()
        sorted_actions = a_pred[0, :].argsort()[::-1]
        for k, idx in enumerate(sorted_actions):
            if idx<100:
                action = self.topo_actions_to_check[idx]
    ...
    def act(
        ...
        if observation.rho.max() > self.rho_danger:
            ...
            # curtailment, storage, redispatching=np.zeros((236,)),np.zeros((236,)),np.zeros((236,))
